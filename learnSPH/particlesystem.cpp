...
void FluidSystem::updateAccelerations(std::vector<BoundarySystem> boundaries, const CompactNSearch::NeighborhoodSearch& nsearch) {
...
Eigen::Vector3d FluidSystem::calculateAccelerationPressure(std::vector<BoundarySystem> boundaries, const CompactNSearch::NeighborhoodSearch& nsearch, const size_t fpI) {
...
Eigen::Vector3d FluidSystem::calculateAccelerationPressureFF(const CompactNSearch::NeighborhoodSearch& nsearch, const size_t fpI, const double particleDensityRatio) {
...
Eigen::Vector3d FluidSystem::calculateAccelerationPressureFS(std::vector<BoundarySystem> boundaries, const CompactNSearch::NeighborhoodSearch& nsearch, const size_t fpI, const double particleDensityRatio) {
    Eigen::Vector3d result = Eigen::Vector3d::Zero();
    if (boundaries.size() > 0) {
        for (const BoundarySystem &boundary : boundaries) {
            ...
            for (size_t fpN = 0; fpN < fluidPS.n_neighbors(boundary.id, fpI); fpN++) {
                ...
                result += this->restDensity * boundary.volumes[fnI] * particleDensityRatio * Kernel::CubicSpline::gradWeight(this->positions[fpI], boundary.positions[fnI], h);
            }
        }
    }
    return result;
...
Eigen::Vector3d FluidSystem::calculateAccelerationViscosity(std::vector<BoundarySystem> boundaries, const CompactNSearch::NeighborhoodSearch& nsearch, const size_t fpI) {
...
Eigen::Vector3d FluidSystem::calculateAccelerationViscosityFF(const CompactNSearch::NeighborhoodSearch& nsearch, const size_t fpI) {
    const double h = Kernel::Parameter::TUNING * this->particleRadius * 2;
    CompactNSearch::PointSet const& fluidPS = nsearch.point_set(this->id);
    Eigen::Vector3d result = Eigen::Vector3d::Zero();
    for (size_t fpN = 0; fpN < fluidPS.n_neighbors(this->id, fpI); fpN++) {
        ...
        result += neighborVol * velocityDiff * posDiff.transpose() * Kernel::CubicSpline::gradWeight(this->positions[fpI], this->positions[fnI], h) * pow(pow(posDiff.norm(), 2) + 0.01 * pow(h, 2), -1);
    }
    return result;
}

Eigen::Vector3d FluidSystem::calculateAccelerationViscosityFS(std::vector<BoundarySystem> boundaries, const CompactNSearch::NeighborhoodSearch& nsearch, const size_t fpI) {
    Eigen::Vector3d result = Eigen::Vector3d::Zero();
    for (const BoundarySystem& boundary : boundaries) {
        ...
        for (size_t fpN = 0; fpN < fluidPS.n_neighbors(boundary.id, fpI); fpN++) {
            ...
            resultBoundary += boundary.volumes[fnI] * this->velocities[fpI] * posDiff.transpose() * Kernel::CubicSpline::gradWeight(this->positions[fpI], boundary.positions[fnI], h) * pow(pow(posDiff.norm(), 2) + 0.01 * pow(h, 2), -1);
        }
...
double learnSPH::ParticleSystem::calculatePressure(const double particleDensity, const double restDensity) {
    return std::max(Parameter::B * (particleDensity - restDensity), 0.0);
}
